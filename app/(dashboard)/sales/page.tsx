      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <Card className="border border-border bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{todaySales.length} {todaySales.length === 1 ? 'bill' : 'bills'} today</p>
          </CardContent>
        </Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/50">
              {sales.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No sales found</td></tr>
              ) : (
                sales.map((sale: any) => {
